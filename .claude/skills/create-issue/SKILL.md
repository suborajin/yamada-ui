...
8. **Present plan for user review**
   ...
   **For the `ai-used` checkboxes field in the template:** copy the options as-is into `--body`, but automatically check the appropriate boxes — do not ask the user:
   - Uncheck `I did not use AI to create this issue.` (leave as `[ ]`)
   - Check `(If there is no check above) I checked the generated content before submitting.` (set to `[x]`)
